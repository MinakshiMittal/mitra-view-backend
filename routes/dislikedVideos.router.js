...
.get(async(req, res)=>{
  ...
  const dislikedVideos = await foundUser.populate("dislikedVideos.video").execPopulate();

console.log(dislikedVideos);
  res.json({success: true, dislikedVideos: dislikedVideos.dislikedVideos})
})
.post(async(req, res) => {
  ...
  const videoPresent = _.some(interactingUser.dislikedVideos, (dislikedVideo) => dislikedVideo.video.toString() === video.video);
  ...
  interactingUser.dislikedVideos.addToSet(video);
...
router.param("videoId", async (req,res,next,videoId)=>{
  try{
    ...
    video = userFound.dislikedVideos;
    console.log(video);
