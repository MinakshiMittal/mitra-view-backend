...
.get(async(req, res)=>{
  ...
  const history = await foundUser.populate("history.video").execPopulate();
...
.post(async(req, res) => {
  ...
  const videoPresent = _.some(interactingUser.history, (historyVideo) => historyVideo.video.toString() === video.video);
  ...
  interactingUser.history.addToSet(video);
